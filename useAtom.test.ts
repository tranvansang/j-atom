...
function nextStateFromAction<T>(action: SetStateAction<T>, state: T): T {
	return typeof action === 'function'
		? (action as (cur: T) => T)(state)
		: action
}
function useRefState<T>(initialValue?: T): [T | undefined, (setStateAction: SetStateAction<T | undefined>) => void, RefObject<T | undefined>] {
...
describe('useAtom', () => {
	...
	test('server-side rendering with renderToString', () => {
		const atom = makeAtom('initial-ssr-value')

		// Create a component that uses the atom
		const TestComponent = () => {
			const value = useAtom(atom)
			return String(value)
		}

		// Update atom value before rendering to test getServerSnapshot
		atom.value = 'updated-before-ssr'

		// Use renderToString to trigger server-side rendering
		// This will call the getServerSnapshot function in useAtom
		const html = renderToString(createElement(TestComponent))

		// The rendered HTML should contain the current atom value
		expect(html).toBe('updated-before-ssr')
	})
})
